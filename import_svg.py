...
def parseAbstractNode(node, context):
    ...
    return None
#------------------------------------CONTRIBUTION BY ANSHUMAN MUND ---------------------------------------------------------------------------------
#helper function
def str_to_index(index):
    if index < 10:
        return "00"+str(index)
...
def create_grease_pencil_from_curve(filepath):
    """
    contribution by anshuman mund
    """
    ...
    object_data_name = "InkscapeSVG"

    svg = xml.dom.minidom.parse(file_path)

    id_layer_dict = {}
    index = 1

    geometries = svg.getElementsByTagName("g")

    #add a grease pencil object and add layers based on geometries
    ...
    #CREATE LAYERS
    if not bpy.data.grease_pencils.get(object_data_name):
        gp_data = bpy.data.grease_pencils.new(object_data_name)
    else:
        gp_data = bpy.data.grease_pencils.get(object_data_name)


        
    if not bpy.data.objects.get(object_name):
        gp_ob = bpy.data.objects.new(object_name, gp_data) 
        context.scene.collection.objects.link(gp_ob)
    else:
        gp_ob = bpy.data.objects.get(object_name)
    ...
    for geometry in geometries:
        #layer stringx
        layer_name = geometry.getAttribute("inkscape:label")
        
        if not gp_data.layers.get(layer_name):
            gpl = gp_data.layers.new(layer_name, set_active = True )
        else:
            gpl = gp_data.layers.get(layer_name)
            
    #DATA BASE OF PATH BASED ON LAYER id_layer_dict
        #get all the paths
        paths = geometry.getElementsByTagName("path")
        for path in paths:
            id_layer_dict[path.getAttribute("id")] = layer_name
    
        #get all the rects
        rects = geometry.getElementsByTagName("rect")
        for rect in rects:
            if not id_layer_dict.get("Curve"):
                id_layer_dict["Curve"] = layer_name
            else:
                id_layer_dict["Curve."+str_to_index(index)] = layer_name
                index+=1
                
        ellipses = geometry.getElementsByTagName("ellipse")
        for ellipse in ellipses:
            id_layer_dict[ellipse.getAttribute("id")] = layer_name
    #create an id data base from svg data
    ...
    #--------------------------------end of creating layers--------------------------------------------------

    #           MANIPULATE   CURVES

    #rotate all the curves

    dumo = bpy.data.collections.get(filename)

    #convert curves to grease_pencil and then rename them
    if dumo:
        for obj in dumo.objects:
            obj.rotation_euler[0] = 3.141/2.0
    ...
    #add all grease pencil objects based on their layer
    for obj_name in id_layer_dict.keys():
        print(obj_name)
        #grab all the points of the GP_Layer
        gp_obj = bpy.data.objects.get(obj_name)
        if gp_obj != None:
            #get all the vertex points
            gplayer = gp_obj.data.layers[0]
            points = gplayer.frames[0].strokes[0].points
        #print(len(points))    
        #add the points to the corresponding layer of Dumo
        gp_layer = bpy.data.objects.get(object_name).data.layers.get(id_layer_dict.get(obj_name))
        
        if gp_layer.frames:
            fr = gp_layer.frames[0]
        else:
            fr = gp_layer.frames.new(1) 
        ...
        #v= [(-20,0,10),(-10,0,5),(20,0,25),(20,0,-25)]
        for i, point in enumerate(ppoints):
            print(i,len(points),len(ppoints))
            ppoints[i].co = points[i].co
        
        #add material 
        mat = bpy.data.objects.get(obj_name).data.materials[0] #get it from the curve 
        
        mat.name = id_layer_dict.get(obj_name)+mat.name
        
        if not bpy.data.objects.get(object_name).data.materials.get(mat.name):
            bpy.data.objects.get(object_name).data.materials.append(mat) 
            
        stroke.material_index = len(bpy.data.objects.get(object_name).data.materials)-1
        #delete the GP object from which copied points
        bpy.data.objects.remove(bpy.data.objects.get(obj_name))
        
        #rotate the object by 90
        bpy.data.objects.get(object_name).rotation_euler[0] = 3.141/2.0
        
        
        
        #next is assigning material
        #first remove all the materials
        
    bpy.data.collections.remove(bpy.data.collections.get(filename))
...
def load_svg(context, filepath, do_colormanage):
    ...
    #MANIPULATE THE SVG CURVE AND CREATE THE GREASE PENCIL OBJECT
    #create the grease pencil svg object.

    create_grease_pencil_from_curve(filepath)
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------
